...
GameView.prototype.render = function (game) {
  ...
  const editButton = this.createEditButton(game._id);
  gameContainer.appendChild(editButton);
...
GameView.prototype.createEditButton = function (gameId) {
  const button = document.createElement('button');
  ...
  button.addEventListener('click', (evt) => {
    PubSub.publish('GameView:game-edit-clicked', evt.target.value);
  });
